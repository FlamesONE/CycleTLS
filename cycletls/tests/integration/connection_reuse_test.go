...
func TestConnectionReuseDisabled(t *testing.T) {
	...
	// Initialize client options with connection reuse DISABLED
	options := cycletls.Options{
		Ja3:                   "771,4865-4866-4867-49195-49199-49196-49200-52393-52392-49171-49172-156-157-47-53,0-23-65281-10-11-35-16-5-13-18-51-45-43-27-17513,29-23-24,0",
		UserAgent:             "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.54 Safari/537.36",
		InsecureSkipVerify:    true,  // Required for test server's self-signed certificate
	...
	// Make second request to the same server (should NOT reuse connection)
	resp2, err := client.Do(serverURL+"/second", options, "GET")
	if err != nil {
		t.Fatalf("Second request failed: %v", err)
	}
	...
	// Make third request to the same server (should NOT reuse connection)
	resp3, err := client.Do(serverURL+"/third", options, "GET")
	if err != nil {
		t.Fatalf("Third request failed: %v", err)
	}
	...
	// For connection reuse DISABLED, we should have:
	// - 4 total requests (3 regular + 1 stats request)
	// - More than 1 handshake since connections are NOT being reused
	if totalRequests != 4 {
		t.Errorf("Expected 4 total requests, got %d", totalRequests)
	}

	// With connection reuse disabled, each request should create a new connection
	// Therefore we expect MORE than 1 handshake (likely 4, one per request)
	if handshakes <= 1 {
		t.Errorf("Expected more than 1 handshake (connection reuse disabled), got %d", handshakes)
	}
