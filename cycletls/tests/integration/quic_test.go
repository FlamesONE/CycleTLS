...
func TestQUICHttp3WithRoundTripper(t *testing.T) {
	...
	if err != nil {
		...
		t.Logf("QUIC HTTP/3 request failed with error: %v", err)
		...
		if errorStr := err.Error(); errorStr != "" {
			...
			// Don't skip the test - we want to see and fix the actual error
			if errorStr == "H3_REQUEST_CANCELLED (local)" {
				t.Errorf("Request was cancelled locally - this indicates a timeout or connection issue that should be fixed")
			} else if errorStr == "failed to sufficiently increase receive buffer size" {
				t.Errorf("UDP buffer size issue - this should be handled properly in QUIC configuration")
			} else {
				// For other errors, we can skip since they might be environment-related
				t.Skipf("QUIC HTTP/3 request failed (this may be expected in some environments): %v", err)
			}
		} else {
			t.Skipf("QUIC HTTP/3 request failed with unknown error: %v", err)
		}
		return
	}
